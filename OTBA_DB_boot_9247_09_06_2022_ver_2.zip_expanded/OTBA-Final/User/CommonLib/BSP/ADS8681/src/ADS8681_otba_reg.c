/**
 ******************************************************************************
 * @file    ADS8681_otba_reg.c
 * @brief   ADS8681 Registers access implementations
 ******************************************************************************
 * @attention
 *
 ...
 *
 *  @param driver_bundle - ADS8681 driver bundle structure pointer.
 *  @param reg_addr - Register address.
 *  @param reg_val - Register value pointer
 *  @return **IO_ERROR_ADS8681__OK** if the registers where read successfully
...
int32_t _ADS8681_otba_read_register(ADS8681_OTBA_DRIVER_BUNDLE_PTR driver_bundle, uint8_t reg_addr, uint8_t *reg_val)
{
	...
	temp[1] = 0;
	temp[0] = 0;
	err = io_write(driver_bundle->PHY.driver, temp, 4);
	...
	// Read the SPI data
	err = io_read(driver_bundle->PHY.driver, temp, 4);
 ...
 *  @return **IO_ERROR_ADS8681__OK** if the registers where written successfully
 *  @return **SPI Error** if SPI error occured
 */
int32_t _ADS8681_otba_write_register(ADS8681_OTBA_DRIVER_BUNDLE_PTR driver_bundle, uint8_t reg_addr, uint8_t reg_val)
{
	...
	temp[0] = reg_addr | ADS8681_OTBA_COMMAND__WREG;
	temp[1] = reg_val;		// dummy

//	printf("ADS8681 Write---> Register: %d, Value 0x%02X\n\r", temp[0], temp[1]);
